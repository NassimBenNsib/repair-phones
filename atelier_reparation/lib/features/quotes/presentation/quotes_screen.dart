...
class QuotesScreen extends ConsumerStatefulWidget {
  ...
  static const String routePath = '/quotes';
...
class _QuotesScreenState extends ConsumerState<QuotesScreen> {
  ...
  List<Quote> _filtered(List<Quote> all) {
    ...
    return all.where((x) {
      final matchStatus = _filter == null || x.status == _filter;
  ...
  Widget _singlePane(BuildContext context) {
    ...
    final results = _filtered(ref.watch(quotesProvider));
                      ...
                      for (final x in results)
                        _QuoteRow(
                          ...
                          onTap: () => Navigator.of(context).push(
                            MaterialPageRoute(
                                builder: (_) =>
                                    QuoteDetailScreen(quoteId: x.id)),
                          ),
                        ),
                    ],
                  ),
...
class _QuoteRow extends StatelessWidget {
  const _QuoteRow(
      {required this.quote, required this.onTap, this.selected = false});
          ...
          child: Padding(
            ...
            child: Row(
              children: [
                ...
                AppleBadge(
                    label: quote.status.label(l),
                    color: quote.status.color(colors)),
                const SizedBox(width: 8),
                Text(AppFormats.money(quote.totals.total, decimals: 0),
